...
const { proto, areJidsSameUser, extractMessageContent, downloadContentFromMessage, getContentType, getDevice } = require('@denzy-official/baileys');
...
async function bgcolor(text, bgcolor) {
  return !bgcolor ? chalk.green(text) : chalk.bgKeyword(bgcolor)(text);
}

async function Lognyong(text, color) {
  return !color
...
async function writeExifWebp(mediaPath, { packname, author }) {
  ...
  const json = {
    'sticker-pack-id': 'akirasquad-' + Crypto.randomBytes(4).toString('hex'),
    'sticker-pack-name': packname,
    'sticker-pack-publisher': author,
    'emojis': ['🔥']
  }

  // format EXIF yang benar (harus pakai header dan panjang JSON)
...
module.exports = { unixTimestampSeconds, generateMessageTag, processTime, webApi, getRandom, getBuffer, fetchJson, runtime, clockString, sleep, isUrl, getTime, formatDate, tanggal, formatp, jsonformat, reSize, toHD, logic, generateProfilePicture, bytesToSize, checkBandwidth, getSizeMedia, parseMention, getGroupAdmins, readFileTxt, readFileJson, getHashedPassword, generateAuthToken, cekMenfes, generateToken, batasiTeks, randomText, isEmoji, getTypeUrlMedia, pickRandom, getAllHTML, toIDR, capital, imageToWebp, videoToWebp, writeExifImg, writeExifVid, writeExif, exifAvatar, addExif, writeExifWebp, color, bgcolor, Lognyong };
