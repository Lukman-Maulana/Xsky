...
case 'rch': {
  try {
    if (!text) return m.reply(`Contoh:\n${prefix + command} <link> <emoji1,emoji2,emoji3>`);

    let [link, ...rest] = text.split(" ");
    if (!link || rest.length === 0) {
      return m.reply(`Format salah!\n${prefix + command} https://whatsapp.com/channel/... 👍,🔥,😂`);
    }

    // gabung kembali sisanya & split koma
    let emojis = rest.join(" ")
      .split(",")
      .map(v => v.trim())
      .filter(v => v.length > 0);

    if (emojis.length === 0) return m.reply("Emoji kosong.");
    ...
    for (let emo of emojis) {
      const api = `https://yaemiko-narukami.vercel.app/tools/rch?link=${encodeURIComponent(link)}&emoji=${encodeURIComponent(emo)}`;
      ...
      switch (status) {
        case 200: {
          ...
          log.push(`✓ ${emo} → ${d.message || "OK"}`);
          sukses++;
          break;
        }
        case 429:
          log.push(`× ${emo} → 429 (Rate Limit)`);
          gagal++;
          break;
        case 400:
          log.push(`× ${emo} → 400 (Link/emoji salah)`);
          gagal++;
          break;
        default:
          log.push(`× ${emo} → ${status}`);
